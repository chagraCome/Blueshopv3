 ...
 * $Author: Montasser $
 */
class Workflow_Backend_Workflow_Add_Controller extends Amhsoft_System_Web_Controller {

  /** @var Workflow_Form $workFlowForm */
  ...
  public function __initialize() {
    $this->workFlowForm = new Workflow_Form('$workFlowForm_form', 'POST');
    $this->workFlowModel = new Workflow_Model();
    $this->getView()->setMessage(_t('Create new Workflow'), View_Message_Type::INFO);
  }
  ...
  public function __default() {
    ...
    if ($this->workFlowForm->isSend()) {
      if ($this->workFlowForm->isValid()) {
        ...
        $this->workFlowModel->setTriggerName($this->workFlowModel->calculateTrigger());
        $e = $workFlowModelAdapter->save($this->workFlowModel);
        $this->handleSuccess();
      } else {

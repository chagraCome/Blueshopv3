 ...
 * $Author: Montasser $
 */
class Workflow_Backend_Action_Modify_Controller extends Amhsoft_System_Web_Controller {

  /** @var Workflow_Action_Form $workFlowForm */
  protected $workFlowForm;

  /** @var Workflow_Action_Model $workFlowModel */
  protected $workFlowModel;

  public function __initialize() {
    $this->workFlowForm = new Workflow_Action_Form('project_form', 'POST');
    $this->getView()->setMessage(_t('Edit Workflow Action'), View_Message_Type::INFO);
    $id = $this->getRequest()->getId();
    if ($id > 0) {
      $workFlowModelAdapter = new Workflow_Action_Model_Adapter();
      $this->workFlowModel = $workFlowModelAdapter->fetchById($id);
    }
    if (!$this->workFlowModel instanceof Workflow_Action_Model) {
      die('Requested Workflow Action not found');
    }

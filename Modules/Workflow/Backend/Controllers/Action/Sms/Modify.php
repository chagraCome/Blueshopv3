 ...
 * $Author: Montasser $
 */
class Workflow_Backend_Action_Sms_Modify_Controller extends Amhsoft_System_Web_Controller {
  ...
  public function __initialize() {
    $this->workFlowForm = new Workflow_Action_Sms_Form('project_form', 'POST');
    $this->getView()->setMessage(_t('Edit Workflow Sms Action'), View_Message_Type::INFO);
    $id = $this->getRequest()->getId();
    if ($id > 0) {
      $workFlowModelAdapter = new Workflow_Action_Sms_Model_Adapter();
      $this->workFlowModel = $workFlowModelAdapter->fetchById($id);
    }
    if (!$this->workFlowModel instanceof Workflow_Action_Sms_Model) {
      die('Requested Workflow Action Sms not found');
    }

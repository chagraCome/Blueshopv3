...
class Crm_Backend_Account_Address_Add_Controller extends Amhsoft_System_Web_Controller {
  ...
  public function __initialize() {
    $this->addressModel = new Crm_Address_Model();
    $this->addressFrom = new Crm_Address_Form('address_form', 'POST');
    $this->accountId = $this->getRequest()->getInt('acc_id');
    if ($this->accountId <= 0) {
      throw new Amhsoft_Item_Not_Found_Exception();
    }
    $this->addressModel->setAccountId($this->accountId);
  }
  ...
  public function __default() {
    $this->getView()->setMessage(_t('Add Address'), View_Message_Type::INFO);
	...
	if ($e > 0) {
	  $this->getRedirector()->go('admin.php?module=crm&page=account-detail&id=' . $this->accountId . '&ret=true');
	} else {

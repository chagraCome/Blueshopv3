 ...
 * $Author: a.cherif $
 */
class Crm_Backend_Contact_Document_Generate_Controller extends Amhsoft_System_Web_Controller {

  public $id;

  /** @var Crm_Contact_Model $model * */
  public $model;

  /** @var Crm_Contact_Document_Model $documentModel * */
  ...
  public function __initialize() {
    ...
    if (!$this->model instanceof Crm_Contact_Model) {
  ...
  public function loadForm() {
    ...
    $templateDirectoryInput->PopUpUrl = 'admin.php?module=setting&page=template-email-quicklist&target=contact&target_id=' . $this->id;
  ...
  public function generate($content) {
    ...
    $documentAdapter = new Crm_Contact_Document_Model_Adapter();
    $documentAdapter->save($this->documentModel);
    if ($this->documentModel->getId() > 0) {
      $pdf->Output('media/contact/docs/' . $this->documentModel->getId() . '.pdf', 'F');
    }
    $saleorderModelAdapter = new Crm_Contact_Model_Adapter();
    $this->model->addDocument($this->documentModel);
    $e = $saleorderModelAdapter->save($this->model);
    $this->getRedirector()->go('admin.php?module=crm&page=contact-detail&ret=true&id=' . $this->id);
  }

 ...
 * $Author: a.cherif $
 */
class Amhsoft_Soap_Server extends nusoap_base {
  /**
   ...
   * @access   public
   */
  function Amhsoft_Soap_Server($wsdl=false){
    parent::nusoap_base();
    // turn on debugging?
    global $debug;
    ...
    if (isset($debug)) {
      $this->debug("In Amhsoft_Soap_Server, set debug_flag=$debug based on global flag");
      $this->debug_flag = $debug;
    } elseif (isset($_SERVER['QUERY_STRING'])) {
      $qs = explode('&', $_SERVER['QUERY_STRING']);
      foreach ($qs as $v) {
        if (substr($v, 0, 6) == 'debug=') {
          $this->debug("In Amhsoft_Soap_Server, set debug_flag=" . substr($v, 6) . " based on query string #1");
      ...
      foreach ($qs as $v) {
        if (substr($v, 0, 6) == 'debug=') {
          $this->debug("In Amhsoft_Soap_Server, set debug_flag=" . substr($v, 6) . " based on query string #2");
          $this->debug_flag = substr($v, 6);
        }
      }
    }
    ...
    // wsdl
    if($wsdl){
      $this->debug("In Amhsoft_Soap_Server, WSDL is specified");
 ...
 */
class soap_server extends Amhsoft_Soap_Server {
}

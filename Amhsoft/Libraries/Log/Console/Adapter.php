<?php
/**
 * NOTICE OF LICENSE
 *
 * This source file is part of AmhsoftFrameWork
 * AmhsoftFrameWork is a commercial software
 *
 * $Id: Adapter.php 102 2016-01-25 21:55:57Z a.cherif $
 * $Rev: 102 $
 * @package    offer
 * @copyright  2005-2010 (c) AMHSOFT e.K. (Web & Software Solutions) Germany (http://www.Amhsoft.com)
 * @license    Amhsoft FrameWork is a commercial software
 * $Date:
 * $LastChangedDate: 2016-01-25 22:55:57 +0100 (lun., 25 janv. 2016) $
 * $Author: a.cherif $
 */
class Amhsoft_Log_Console_Adapter extends Amhsoft_Log_Abstract{
    
    public static function error($msg, $obj=null) {
        print 'error ('.date('Y-m-d H:i:s').'): '.$msg." \n";
    }

    public static function info($msg, $obj=null) {
        print 'info ('.date('Y-m-d H:i:s').'): '.$msg." \n";
    }

    public static function warn($msg, $obj=null) {
        print 'warn ('.date('Y-m-d H:i:s').'): '.$msg." \n";
    }
}
?>

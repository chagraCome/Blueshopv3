...
class Amhsoft_Sms_Gateway_MobilyWs_Adapter extends Amhsoft_Sms_Gateway_Abstract {

    protected $username;
    protected $password;
    protected $sender;

    const URL = 'http://www.mobily.ws/api/msgSend.php';
    ...
    public function getBalance() {
        ...
        $http->useCurl(false);
        return $http->execute($url, null, 'GET', array('mobile' => $this->username, 'password' => $this->password));
    }
    ...
    /**
     * Send Sms.
     * @param string $to mobile number or many mobile numbers separeted with comma ","
    ...
    public function send($to, $message) {
        $http = new Amhsoft_Http();
        $http->useCurl(false);
        return $http->execute(self::URL, null, 'GET', array('mobile' => $this->username, 'password' => $this->password, 'numbers' => $to, 'sender' => urlencode($this->sender), 'msg' => $this->convertToUnicode($message), 'applicationType' => '24', 'timeSend' => '0'));
    }
    ...
    private function convertToUnicode($message) {
        ...
        $strResult = "";
        for ($i = 0; $i < mb_strlen($message, "UTF8"); $i++) {
            if (in_array(mb_substr($message, $i, 1, "UTF8"), $chrArray))
                $strResult.= $unicodeArray[array_search(mb_substr($message, $i, 1, "UTF8"), $chrArray)];
        }
        return $strResult;
    }

...
/**
 * Amhsoft_DataGridView_Component class
 * @author Amir Cherif
 */
class Amhsoft_Widget_DataGridView implements Amhsoft_Widget_Interface {
    ...
    private function buildPagination() {
        ...
        $margin = (Amhsoft_System::getCurrentLang('lang') == 'ar') ? 'margin-right: 250px;float:left;direction:rtl' : 'margin-left: 250px; float:right';
        ...
        return $action . '<h4 ' . $dir . ' align="' . $align . '" style="padding-bottom:10px;' . $margin . '">' . _t('Page') . ' ' . ($this->getPageNumber() + 1) . ' ' . _t('from') . ' ' . $this->pageCount() . '&nbsp;&nbsp;&nbsp;&nbsp;' . _t('Total Records') . ': ' . $this->getTotal() . '
            ' . $selectBox->Render() . '
    ...
    private function init() {
        ...
        $table .= '<table id="'.$this->Id.'"  class="' . $this->Class . '" width="' . $this->Width . '" ' . $this->Style . ' >';
        return $table;
    }
...
//    }
    ...
    private function getHeaderColumnHtml($data, $index = null) {
        ...
        if ($data instanceof Amhsoft_CheckBox_Control && !isset($data->notCheck)) {
            ...
            return '<th width="40">' . $checkAll->Render() . '</th>';
        }
        ...
        if ($this->Sortable == true) {
            if ($data->DataBinding->Value != $data->DataBinding->Index) {
                return '<th ' . $width . '><a href="' . $this->getSortUrl($data->DataBinding->Index) . '">' . $data->Label . '</a></th>';
            }
            return '<th ' . $width . '><a href="' . $this->getSortUrl($data->DataBinding->Value) . '">' . $data->Label . '</a></th>';
        }

        return '<th ' . $width . '>' . $data->Label . '</th>';
    }
    ...
    public function Render() {
        ...
        if ($this->pagination) {
            $str .= $this->pagination->toHTML($this->LinkUrl);
        }
    ...
    public function buildSearchFields() {
        ...
        $str = '<tbody><tr class="table_search">';
        $i = 0;
        while ($i < count($this->columns)) {

...
class CartCest {
    ...
    // want to check Cart Setting
    public function CartSetting(AcceptanceTester $I) {
        $I->wantToTest('Cart Setting');
        $I->wantTo('go to /admin.php?module=cart&page=setting');
        $I->amOnPage('/admin.php?module=cart&page=setting');
        $I->wantTo('check if page reachable');
        $I->seeResponseCodeIs(200);
        $I->wantTo('check notfound not exist in url');
        $I->dontSeeInCurrentUrl('notfound');

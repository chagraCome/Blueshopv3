...
class SettingCest {
    ...
    // want to check Sale order Setting
    public function SaleOrderSetting(AcceptanceTester $I) {
        $I->wantToTest('banner Setting');
        $I->wantTo('go to /admin.php?module=saleorder&page=setting');
        $I->amOnPage('/admin.php?module=saleorder&page=setting');
        $I->wantTo('check if page reachable');
        $I->seeResponseCodeIs(200);
        $I->wantTo('check notfound not exist in url');
        $I->dontSeeInCurrentUrl('notfound');

...
class InstallerCest {
    ...
    // want to test Modules Management
    public function ModulesManagement(AcceptanceTester $I) {
        $I->wantToTest('Modules Management');
        $I->wantTo('go to /admin.php?module=installer&page=list');
        $I->amOnPage('/admin.php?module=installer&page=list');
        $I->wantTo('check if page reachable');
        $I->seeResponseCodeIs(200);
        $I->wantTo('check notfound not exist in url');
        $I->dontSeeInCurrentUrl('notfound');

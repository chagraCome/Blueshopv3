...
class PaypalCest {
    ...
    // want to check Paypal Settings
    public function PaypalSettings(AcceptanceTester $I) {
        $I->wantToTest('Paypal Settings');
        $I->wantTo('go to /admin.php?module=paypal&page=settings');
        $I->amOnPage('/admin.php?module=paypal&page=settings');
        $I->wantTo('check if page reachable');
        $I->seeResponseCodeIs(200);
        $I->wantTo('check notfound not exist in url');
        $I->dontSeeInCurrentUrl('notfound');
